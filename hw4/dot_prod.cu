...
void reduction(double* dot_ptr, const double* a, const double* b, long N){
  double sum = 0;
...
__global__ void init_reduction_kernel(double* sum, const double* a, const double* b, long N){
...
int main() {
  ...
  init_reduction_kernel<<<Nb,BLOCK_SIZE>>>(sum_d, x_d, y_d, N);
  while (Nb > 1) {
    long N = Nb;
