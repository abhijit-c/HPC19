...
//
{
  ...
  // Valgrind indicates that unitialized memory created here is used later in a
  // conditional jump, i.e. the print statement. We must modify the for to
  // properly initialize.
  x = new int[10];
//
//  X = { 0, 1, 2, 3, 4, ?a, ?b, ?c, ?d, ?e }.
//
  //for ( i = 0; i < 5; i++ ) Now no errors =)
  for ( i = 0; i < 10; i++ )
  {
    x[i] = i;
  }
